            ...
            {data?.map((item, index) => (
                <div key={index} className={styles['shop-product__item']}>
                    <div className={styles['shop-product__item-img']}>
                        <span className={`${styles['shop-product__item-float']} ${styles['shop-product__item-float--tag-sale']}`}>
                            {(item.salePrice) ? <span className="product-tag product-tag--sale">{formatPercent(item.salePrice, item.regularPrice)}</span> : ""}
                        </span>
                        <button className={`${styles['shop-product__item-float']} ${styles['shop-product__item-float--action']}`}>
                            <Icon.HeartFill />
                        </button>
                    ...
                    <div className={styles['shop-product__item-text']}>
                        <h3><Link href={`${path.public.productRoute}/${item._id}`}><a href="" className={styles["shop-product__item-title"]}>{item.name}</a></Link></h3>
                        <div className={styles['shop-product__item-row']}>
                            ...
                            <div className={styles['shop-product__item-variation']}>
                                {(item.size.length > 3 ? <div><span className={styles['shop-product-variation__item']}>{item.size[0]}</span>
                                    <span className={styles['shop-product-variation__item']}>{item.size[1]}</span>
                                    <span className={styles['shop-product-variation__item']}>{item.size[2]}</span>
                                    <span>...</span>
                                </div> : item.size.map((sizeItem) => (
                                    <span className={styles['shop-product-variation__item']}>{sizeItem}</span>
                                )))}
                        ...
                        <div className={styles['shop-product__item-row']}>
                            ...
                            <Button.Transparent className="!tw-py-[1px] !tw-px-1 !tw-min-h-0 !tw-h-[32px]" content={<Icon.PlusRegular className="tw-text-2xl" />} />
                        </div>
                    </div>
                </div>
            ))}
