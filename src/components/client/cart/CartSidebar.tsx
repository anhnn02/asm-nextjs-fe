        ...
        <div className="tw-h-screen tw-menu tw-overflow-y-auto tw-w-[400px] tw-bg-base-100 tw-text-base-content">
          <div className={styles["cart-sidebar__header"]}>
            ...
            {cartTotalQuantity == 0 ? (
              ...
              <button
                onClick={() => removeCart()}
                className="tw-text-my-gray tw-font-normal"
              >
                Clear All
              </button>
            )}
            ...
            <>
                ...
                {cart.map((item, index) => (
                  <div key={index} className={styles["cart-sidebar__item"]}>
                    <div className={styles["cart-sidebar__action-quantity"]}>
                      <button
                        className={styles["cart-sidebar__button"]}
                        onClick={() =>
                          dispatch(
                            incrementQuantity({
                      ...
                      <span className={styles["cart-sidebar__quantity"]}>
                        {item.quantity}
                      </span>
                      <button
                        className={styles["cart-sidebar__button"]}
                    ...
                    <div className={styles["cart-sidebar__info"]}>
                      ...
                      <Link href={`{path.private.productRoute/${item.idPro}`}>
                        <img
                          className={`${styles["cart-sidebar__img"]} && tw-cursor-pointer`}
                          src={item.img}
                          alt=""
                        />
                      </Link>
                      <div className={styles["cart-sidebar__text"]}>
                        <h3>
                          <Link
                            href={`{path.private.productRoute/${item.idPro}`}
                          >
                            <a
                              href=""
                              className={styles["cart-sidebar__title"]}
                            >
                              {item.name}
                            </a>
                          </Link>
                        ...
                        <span className={styles["cart-sidebar__price"]}>
                          ...
                          {item?.salePrice ? (
                            <span className="tw-line-through tw-pl-1">
                              {formatPrice(item?.regularPrice)}
                            </span>
                          ) : (
                            ""
                        ...
                        <span className={styles["cart-sidebar__size"]}>
                          Size: {item.size}
                        </span>
                        <span className={styles["cart-sidebar__sub-total"]}>
              ...
              {cartTotalQuantity == 0 ? (
                ...
                <div className={styles["cart-sidebar__footer"]}>
                  <Button.Fill
                    onClick={() => checkUser()}
                    className="tw-block tw-w-full !tw-h-3 !tw-p-1"
                    content={`Checkout now ${formatPrice(totalAmount)}`}
                  />
                  <Button.Transparent
                    className="tw-block tw-w-full !tw-h-3 !tw-p-1"
                    content={
