    ...
    <div className={styles["header"]}>
      ...
      <div className={styles["header-main"]}>
        <div className={styles["header-main__top"]}>
          ...
          <form
            action=""
            className={styles["header-form-search"]}
            onSubmit={handleSubmit(getValueSearch)}
          >
            <input
              type="text"
              className={styles["header-form-search__input"]}
              {...register("keyword")}
              required
            />
            <button className={styles["header-form-search__button"]}>
              Search
            </button>
          </form>
          <div className={styles["header-action"]}>
            {isLogin === false ? (
              <div>
                <Link href={path.public.signupRoute}>
                  <a className={styles["header-action-link"]}>Register</a>
                </Link>
                <span className="tw-px-2">/</span>
                <Link
                  className={styles["header-action-link"]}
                  href={path.public.loginRoute}
                >
                  <a className={styles["header-action-link"]}>Sign in</a>
                </Link>
              </div>
            ) : (
              <button className="tw-dropdown tw-dropdown-end">
                ...
                <ul className="tw-dropdown-content tw-menu tw-p-2 tw-shadow tw-bg-base-100 tw-rounded-box tw-w-52 tw-capitalize tw-font-semibold">
                  <li className="tw-truncate">
                    <p>
                      <Icon.Person className={""} /> {userInfor.user.name}
                    </p>
                  </li>
                  <li className="">
                    <Link href={path.public.ordersRoute}>
                      <p>
                        <Icon.Order className={""} /> My orders
                      </p>
                    </Link>
                  </li>
                  <li className="">
                    <p className="" onClick={() => btnLogOut()}>
                      <Icon.BoxArrowLeft className={""} /> Log out
                    </p>
                  </li>
                </ul>
              </button>
            ...
            <div className="tw-indicator">
              <span className="tw-indicator-item tw-badge tw-badge-secondary tw-bg-primary">
                {totalQuantity}
              </span>
              <label
                htmlFor="my-drawer-4"
                className="drawer-button btn btn-primary tw-cursor-pointer"
              >
            ...
            {MenuList.map((menu, index) => (
              <li key={index} className={styles["menu__item"]}>
                <CustomLink
                  ...
                  href={menu.path}
                >
                  <a className={styles["menu__item-link"]}>{menu.label}</a>
                </CustomLink>
              </li>
            ))}
          </ul>
